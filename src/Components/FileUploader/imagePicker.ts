...
const compress = async (file: any, ratio: number) => {
...
const imagePicker = async (file: File) => {
    if (!file) return;
    ...
    if (ratio > 0) {
        const base64 = await compress(file, ratio) as string;
        return dataURItoBlob(base64 as string);
    } else {
        return file
    }
}
const sizeCheck = (size: number) => {
    let res = 0;
