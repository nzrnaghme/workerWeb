...
type Props = {
  onFilter: (filter: IFilterServisecRequests) => void;
  userId: string;
  lastFilter?: IFilterServisecRequests | null;
};
...
function Filters({ onFilter, userId, lastFilter }: Props) {
  ...
  const [status, setStatus] = useState<ICheckable[]>(selectReportRequestStatus);
  ...
  const LastFilter = (lastFilter: IFilterServisecRequests) => {
    ...
    if (lastFilter.reportRequestStatus) {
      const categoryUserSelectLast = lastFilter.reportRequestStatus;
      StatusUser = selectReportRequestStatus.map((i) => {
  ...
  const onFilterApplied = (
    ...
    const filter: IFilterServisecRequests = {
      skip: 0,
      take: 12,
      userId: userId,
      ...
      reportRequestStatus: idsListMaker(status),
    };
    ...
    <>
      {hasCategory && (
        <div className="filters-container">
          ...
          <DateFilter
            LastSelect={date}
            titleName="تاریخ پذیرش درخواست"
            onDateFiltered={(c: any) => {
              onFilterApplied(
                categories,
                reqTypes,
                status,
